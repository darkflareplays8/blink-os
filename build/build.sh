...
cp -r /config/branding "$BUILD_DIR/config/includes.chroot/usr/share/blink/"
...
VBoxManage createvm --name "BlinkOS" --ostype "Debian_64" --register
...
VBoxManage export "BlinkOS" \
    --output "$OUTPUT_DIR/$DISTRO_NAME-$DISTRO_VERSION.ova" \
    --ovf20 \
    --manifest \
    --options manifest
...
echo "$DISTRO_NAME-$DISTRO_VERSION.ova" > "$OUTPUT_DIR/ova_name.txt"